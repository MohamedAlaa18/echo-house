...
function Cart() {
    ...
    const deleteCartItem = (id: number) => {
        CartApis.deleteCartItem(id).then((res: { data: { data: { id: any; }; }; }) => {
            if (res) {
            ...
            <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
                <div className="mx-auto max-w-3xl">
                    ...
                    <div className="mt-8">
                                ...
                                cart.map((item) => (
                                    <li key={item?.id} className="flex items-center gap-4">
                                        <Image
                                            ...
                                            alt=""
                                            className="size-16 rounded object-cover w-[130px] h-[90px]"
                                        />
                                        ...
                                        <div className="flex flex-1 items-center justify-end gap-2" onClick={() => deleteCartItem(item?.id)}>
                                            <dd className="inline">{item?.product?.attributes?.price} EGP</dd>

                                            <button className="text-gray-600 dark:text-gray-500 transition hover:text-red-600 dark:hover:text-red-500">
                        ...
                        <h2 className='text-gray-400 dark:text-gray-500 text-[12px]'>Note: All Items will be sent via Email</h2>
                    </div>
                </div>
            </div>
        </section >
